...
| Devnet program — **MVP (prod, Railway)** | `AuXJKpuZtkegs2ZSgopgckhN7Ev8bUz4zBc238LD2F1` |
| Devnet program — **Phase 1 (local dev)** | `3YpkUKBh8288XN2dCKSwBnEdyc5UozSJ19A1ZCLpUZsZ` — the Phase 1 indexer + Postgres run locally; ingestion via a tunnel + Helius dev webhook. Promoted at the Stage F ceremony. |
| Permissionless drivers | 5 cron drivers (auto-claim, VRF-reveal, oracle-relayer, partial-refund, close-terminal) — see [Permissionless drivers](#permissionless-drivers) |
| Identity | indexer-owned Steam OpenID → SAS attestation issuance (V1.1) |
| License | MIT |
...
| `GET` | `/health` | Liveness + reconciliation snapshot — `lastReconcileAt`, scanned/touched counts, `lastReconcileError`, `unknownEventVersion`. |
| `POST` | `/webhooks/helius` | Helius webhook ingest. HMAC-SHA256 protected — see [Webhook security](#webhook-security) below. |
| `GET` | `/identity/steam/login` | Begins the Steam link: verifies a wallet Ed25519 sig over `bracketchain:bind-steam:<wallet>:<nonce>`, then 302 → Steam OpenID. |
| `GET` | `/identity/steam/callback` | Server-side `check_authentication`, consumes the single-use nonce, issues the SAS attestation (CPI, `sas-issuer` role), 302 back with `?steam=`. |
| `GET` | `/identity/:wallet/:game` | Read-only attestation PDA lookup for a wallet + game. |
...
curl -s "$INDEXER/tournaments/check-name?organizer=3YpkU...&name=TestCup" | jq .
# { "taken": false }
curl -s "$INDEXER/tournaments/3YpkU.../payouts" | jq '.[] | {kind, amount, recipient}'
...
| `ResultProposed` | Write the `Match` proposal envelope (source/proposer/proposed winner/deadline) — drives the auto-claim driver + UI dispute window |
| `ResultDisputed` | Mark the match disputed, re-arm the deadline (+24h) |
| `ResultClaimed` | Finalize state after a permissionless claim |
...
Every event's first field is `event_version: u8`. The parser rejects any payload where `event_version != EVENT_VERSION_V1 (1)` and increments an `unknownEventVersion` counter surfaced on `/health` — so a future on-chain event-schema bump can't silently corrupt rows.
...
## Permissionless drivers

Phase 1 adds five cron-driven "drivers" that submit **permissionless** instructions on behalf of the protocol, each signed by a named funded keypair role (resolved through `solana-keychain`). All subclass `PermissionlessDriver` (`src/drivers/permissionless-driver.ts`) and call the on-chain program through the pnpm-linked `@bracketchain/sdk`. Each is independently feature-flagged (default-off where it touches funds).

| Driver | Role | Cadence | Scans for → calls |
|---|---|---|---|
| `AutoClaimDriver` | `claim-payer` | every minute | undisputed proposals past `claim_deadline` → `claim_result`. (Final-match scope: `WinnerTakesAll` only; Standard/Deep finals are organizer-adjudicated via the UI.) |
| `VrfRevealDriver` | `vrf-payer` | every minute | tournaments with a bound randomness account, `!seed_revealed`, past the commit slot → `reveal_seed` |
| `OracleRelayerDriver` | `claim-payer` | every minute | matches with a bound feed + fresh update + no proposal → `propose_result_oracle` |
| `PartialRefundDriver` | `refund-payer` | every minute | `PartialCancelled` tournaments with unrefunded participants → `partial_refund_chunk` |
| `CloseTerminalDriver` | `cleanup-payer` | hourly | terminal tournaments older than 7 days → `close_tournament` chunks (rent → organizer) |

---

## Schema

4 Prisma models + 7 enums in [`prisma/schema.prisma`](./prisma/schema.prisma). Phase 1 extended the existing models in place (no new tables — per the "no custom Notification table" decision; intermediate `PendingConfirmation` / `Disputed` UI states are derived from the settlement envelope):
...
└── settlementMode? (enum), game? (enum)              ← V1.1, reconciliation-backfilled

Participant — @@unique(tournamentAddress, wallet)
├── wallet, seedIndex, refundPaid
├── registeredTxSig, chainSlotAtWrite
└── identityHash?, wins, losses, pointsFor, pointsAgainst   ← V1.1 identity + stats

Match — @@unique(tournamentAddress, round, matchIndex)
├── playerA?, playerB?, winner?
├── status (enum: Pending | Active | Completed | Disputed), bye
├── reportedTxSig?, chainSlotAtWrite
├── proposalSource (enum), proposer?, proposedWinner?, claimDeadline?, disputed, disputeReason?   ← Stage B envelope
└── playerAGameId?, playerBGameId?, switchboardFeed?   ← Stage C oracle commitment
...
Enums: `TournamentStatus` (+`PartialCancelled`), `MatchStatus` (+`Disputed`), `PayoutPreset`, `PayoutKind`, plus the V1 additions `SettlementMode`, `Game`, `ProposalSource`.
...
| `20260526213300_b14_settlement_envelope` | Stage B — proposal envelope columns |
| `20260526230613_add_tournament_settlement_mode` | Stage B — `SettlementMode` |
| `20260528120000_stage_c_oracle` | Stage C — oracle commitment columns |
| `20260529000000_add_tournament_game` | V1.1 — `Game` |
| `20260529100000_add_partial_cancelled` | Stage E — `PartialCancelled` status |

`pnpm prisma migrate dev` to apply locally; `start:prod` runs `prisma migrate deploy` before `node dist/main` (see [`Procfile`](./Procfile) for the Railway entrypoint). The last five (Stage B–E) migrations are applied locally for Phase 1 dev; deploying them to the Railway/prod DB is the Stage F **F-6** step.
...
| `PROGRAM_ID` | `chain-reader.service.ts`, `helius-parser.service.ts` | Program PDA — `AuXJKpuZtkegs2ZSgopgckhN7Ev8bUz4zBc238LD2F1` for devnet |
| `HELIUS_WEBHOOK_SECRET` | `helius-hmac.guard.ts` | Shared secret for HMAC-SHA256 verification on `/webhooks/helius`. **Fail-closed** — when unset, every webhook is rejected with 401. See [Webhook security](#webhook-security). |

Optional:

| Var | Read by | Default |
|---|---|---|
| `PORT` | `main.ts` | `3000` (Railway sets this) |
| `FRONTEND_ORIGIN` | `main.ts` | `*` — comma-separated allowlist; lock down before prod |
| `TOKEN_MINT_FILTER` / `USDC_MINT` | `helius-parser.service.ts` | unset — apply only if you want to skip non-USDC payouts. **Leave unset for the multi-token MVP.** |
...
Unit coverage (41 tests across the three production services):

| Spec | Count | Covers |
|---|---|---|
| `src/webhooks/helius-parser.service.spec.ts` | 26 | Every typed event handler — `TournamentCreated`, `ParticipantRegistered`, `TournamentStarted`, `MatchReported`, `TournamentCompleted`, `TournamentCancelled`, `RefundIssued`. Verifies row writes, idempotency, and the discriminated-union `BracketChainEvent` dispatch shape. |
| `src/reconciliation/reconciliation.service.spec.ts` | 12 | Cron drift detection — terminal-status skips, slot-delta thresholds, `getMultipleAccountsInfo` batching, `chainSlotAtWrite` bumps. |
| `src/tournaments/tournaments.service.spec.ts` | 3 | REST controller wrappers + `serializeBigInts` boundary. |

E2E + scaffold: `test/app.e2e-spec.ts` (2 tests) and the auto-generated `src/app.controller.spec.ts` (1 test) are still mostly placeholder. Pre-mainnet hardening item.
...
├── scripts/
│   ├── sync-idl.mjs         # copy IDL from program target/idl/ for the BorshCoder event-decode path (manual)
